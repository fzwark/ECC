...
class Data:
    ...
    def load_data(self,  pairs=1, data = "sprout"):
        if data == "sprout":
            if not os.path.exists(f"./sprout_data_{pairs}/"):
                from itertools import combinations
                ...
                dataset.save_to_disk(f"./sprout_data_{pairs}/")
            else:
                dataset = load_from_disk(f"./sprout_data_{pairs}/")
            ...
            if not os.path.exists(f"./leaderboard_data_{pairs}/"):
                from itertools import combinations
                rng = random.Random(42)
                all_pairs = list(combinations(self.models, 2))
                ...
                def compute_field(example):
                    ...
                    return example
                dataset = self.dataset.map(compute_field) 
                dataset.save_to_disk(f"./leaderboard_data_{pairs}/")
            else:
                dataset = load_from_disk(f"./leaderboard_data_{pairs}/")
